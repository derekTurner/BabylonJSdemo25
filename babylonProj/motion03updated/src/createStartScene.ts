...
function loadAssetContainer(scene: Scene): Promise<AssetContainer> {
  const assetsManager = new AssetsManager(scene);
  let loadedContainer: AssetContainer | null = null;

  const containerTask: ContainerAssetTask = assetsManager.addContainerTask(
    "xbot",
    "xbot",
    "./assets/models/men/",
    "Xbot.glb"
  ...
  containerTask.onSuccess = function (task) {
    loadedContainer = task.loadedContainer;
  };
...
function instantiateCharacter(
  ...
  scene: Scene,
  modelName: string = "xbot"
): AbstractMesh {
  // Store original animation names before instantiation
  const originalAnimNames = assetContainer.animationGroups.map(ag => ag.name);
  
  const instance = assetContainer.instantiateModelsToScene(
    (name: string) => modelName
  );
  const player = instance.rootNodes[0] as AbstractMesh;

  // Restore original animation names to the instantiated groups
  instance.animationGroups.forEach((ag, index) => {
    if (index < originalAnimNames.length) {
      ag.name = originalAnimNames[index];
    }
  });
  ...
  console.log("Animation groups:", instance.animationGroups.map(ag => ag.name));
...
export default async function createStartScene(engine: Engine) {
  ...
  const assetContainer = await loadAssetContainer(scene);
  const player = instantiateCharacter(assetContainer, scene);
